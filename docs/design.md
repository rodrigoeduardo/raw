...
10. **Evidence over assertion for user-visible work.** A green test proves the logic ran, not that anything rendered. When an issue is user-visible, "done" requires a screenshot of the real app driving the real flow — and, critically, that *someone other than the worker looked at it* (the reviewer does, before the verdict). Inert for headless repos.
